...
_G.applyPetSkinLook = function(model, petId, lite)
	...
	if e then
		-- SKIN AND TRAIT ARE INDEPENDENT LAYERS, and the trait must not be gated on the skin. This used to read
		-- `if e and e.skin then` -- both calls inside one test -- which was harmless while a trait could only
		-- arrive welded to a skin inside an inventory key. The Pet Hut now sells traits on their own, so
		-- "trait, no skin" is the state a player is in the moment they buy their first one, and gating it here
		-- meant the hat they just paid for rendered on the preview card and nowhere else.
		if e.skin then applySkin(model, petId, e.skin, lite) end
...
local function applyState(state)
	...
	for petId, e in pairs(state.equipped or {}) do
		if type(e) == "table" then
			...
			local skin = (type(e.skin) == "string") and PetSkins.normalise(e.skin) or nil
			local trait = PetTraits.normalise(e.trait or (type(e.traits) == "table" and e.traits[1]) or "")
			if trait == "" then trait = nil end
			-- EITHER LAYER ALONE IS A REAL EQUIP. The outer test used to require a string skin, which silently
			-- dropped every "trait, no skin" pet -- the exact state the Pet Hut's trait shop produces -- so a
			-- bought hat never reached `equipped` and never painted. A pet with neither is simply not listed.
			if skin or trait then
				equipped[petId] = { skin = skin, trait = trait }
			end
		end
	end
	...
	for petId, e in pairs(equipped) do
		-- Both halves are optional now (a pet can wear a trait and no skin), so neither is concatenated raw.
		sigParts[#sigParts + 1] = petId .. "=" .. (e.skin or "") .. "|" .. (e.trait or "")
	end
	table.sort(sigParts)
