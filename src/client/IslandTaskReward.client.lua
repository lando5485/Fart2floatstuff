--======================================================================
-- IslandTaskReward.client.lua  (LocalScript)
--======================================================================
-- THE PAYOFF MOMENT. You finish an island's pet quest, the egg hatches -- and the game stops and makes a
-- fuss: gold wash, a card that punches in, the token number counting up from zero, confetti, and a stream of
-- tokens that fly across the screen and land in the token pill in the corner.
--
...
local function celebrate(island, islandName, tokens, petName)
	...
	local where = label(card, islandName, 22, CREAM)
	...
	local who = label(card, petName .. " joined you!", 17, LIME)
	who.Size = UDim2.new(1, -28, 0, 24)
	who.Position = UDim2.new(0.5, 0, 0, 212)
	who.AnchorPoint = Vector2.new(0.5, 0)
	who.ZIndex = 5
	fit(who, 17, 11)
	...
	print(string.format("[IslandTask] reward moment played -- island %d (%s), +%d tokens",
		island, tostring(islandName), tokens))
end
...
task.spawn(function()
	...
	ev.OnClientEvent:Connect(function(island, islandName, tokens, petName)
		island = tonumber(island) or 0
		tokens = tonumber(tokens) or 0
		if tokens <= 0 then return end
		-- Half a second of air. The server fires this the instant the pet is claimed, which is the same beat
		-- the hatch animation finishes on -- landing the card on top of the pet reveal steps on both.
		task.delay(0.5, function()
			celebrate(island, tostring(islandName or "Island"), tokens, tostring(petName or "Your pet"))
		end)
	end)
