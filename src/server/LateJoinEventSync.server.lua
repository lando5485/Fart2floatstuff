...
--======================================================================
-- LATE-JOINER CATCH-UP for the server-wide BIG EVENTS (Rocket / Meteor / UFO / Ice Age / Mutation).
--
...
--   * SOUND rides along with the visuals: the looping ambients (UFO eerie/buzz, Ice Age wind/crack,
--     Mutation alarm/bubble) are CLIENT-created in startSky on "start", and Rocket's construction loop is
--     client-played on "constructionStart" -- both are recreated by the phases we replay. The server-made
--     Sounds (Meteor intro, UFO/Mutation server loops, Rocket countdown, all in Workspace) already
--     replicate to late-joiners on their own, so no separate sound mechanism was needed.
--   * 3D content: the Rocket build is ModelStreamingMode.Persistent (replicates regardless of join time);
--     Ice Age snow + Mutation radiation particles are CLIENT-generated by the handlers we replay.
--
...
local syncs = {
	...
	ufo      = ReplicatedStorage:WaitForChild("UFOSync", 30),
	iceage   = ReplicatedStorage:WaitForChild("IceAgeSync", 30),
	mutation = ReplicatedStorage:WaitForChild("MutationSync", 30),
}
...
local CATCHUP = {
	...
	ufo      = { "start", "main" },  -- alien green/purple sky
	iceage   = { "start", "main" },  -- winter sky + guarded physics, then the snow density
	mutation = { "start", "main" },  -- toxic sky + radiation particle volume (built on "start")
}
