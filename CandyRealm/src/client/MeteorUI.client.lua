...
-- 0.17, NOT 0.05. At 0.05 (with height 0.08) this stack occupied y 54..140px on a 1042-tall screen --
-- straight through NotifyCenter's hero banner (y 10..75) AND the ObjectiveHUD card (y 85..163), which
-- has a HIGHER DisplayOrder and so drew right over the meteor text. The shared top-centre budget is:
--   y  10..75   hero banner (NotifyCenter)
--   y  85..163  objective card (ObjectiveHUD)
--   y 175..     this event stack
-- Keep this value in step with RocketUI, which shares the same _G.__eventBannerSlots allocator.
...
local function snapshotLighting()
	...
	-- Tell SkyByAltitude to stand down: it drives ClockTime/Brightness/OutdoorAmbient by altitude, and if it
	-- kept writing them while we own the sky it would fight this event AND we'd snapshot ITS values as the
	-- "originals" -- restoring the player into a night sky at ground level after the meteor ended.
	_G.meteorEventActive = true
	savedLighting = {
...
local function restoreSky()
	...
	-- Hand the sky back. Released only AFTER the restore tween is queued, so SkyByAltitude can't start writing
	-- Lighting mid-restore and fight the tween on its way back to the originals.
	task.delay(2.6, function() _G.meteorEventActive = false end)
