--======================================================================
-- Gifting.client.lua  (LocalScript)  -- CandyRealm
--======================================================================
...
--
-- ===== IT OPENS FROM MORE+ =====
-- The Gift card lives in MorePopup's MORE+ grid beside Rebirth / Fast Travel / Rewards -- and it is what
-- finally makes that grid a real 2x2 instead of three cards and a hole.
--
-- MorePopup calls _G.openGiftPanel, which OPENS rather than toggles: a card in that menu always means "show
...
local function build()
	...
	-- (This replaces the older findCloseX plan the comment above still described.)
	task.defer(function()
		for _ = 1, 40 do
...
--
-- SNAPSHOT AND RESTORE, exactly the way CrateClient does it here. (The Food Realm additionally needed
-- _G.hudHold, because its CoreClient re-asserts BottomStackGui four times a second to beat stale duplicate
-- scripts; CandyRealm has no such authority, so setting Enabled and putting it back is enough. The pcall on
-- _G.hudHold is kept anyway -- harmless if absent, correct if one is ever added.)
...
GiftResult.OnClientEvent:Connect(function(ok, msg)
	if ok then
		...
		-- status line under the SEND button already says it worked -- a gold banner at the top of the screen
