...
local CONFIG = {
	-- ===== IDS =====
	-- These Fart to Float ids are CORRECT here and do not need replacing. CandyRealm is a
	-- separate PLACE inside the same EXPERIENCE, not a separate experience -- monetization is
	-- per-experience, so the ids carry across places. Proof: Shop_AllInOne has been prompting
	-- MidAirRecharge / SkipIsland / BirdNuke with these exact ids in this realm all along.
	productIds = {
	...
	-- ON. The three impulse buttons prompt the ids above, which are valid in this place (see
	-- the note on productIds). Set false for a stats-only panel -- it shrinks to 230x195 and
	-- drops the divider and buttons.
	--
	-- Prompting is only half of it: the SERVER still has to grant. The grants live in Fart to
	-- Float's PlayerStats.server.lua ProcessReceipt, which does NOT run in this realm -- so if
	-- a purchase here takes Robux without granting, that is the missing half, not these ids.
	-- Worth testing one before shipping.
	showImpulseButtons = true, -- panel is 230x500: stats, divider, then the three buttons
	midAirFreezeWhileBuying = true, -- hold the player in place while the Robux prompt is up

	-- The progress bar at the bottom of the stats section.
	progress = {
		-- NOT "TO SPACE REALM": CandyRealm has no Space Realm portal (WorldClient logs
		-- "island 14 not found -- skipping the black hole"), so naming it that points at
		-- somewhere the player cannot go.
		title      = "\xF0\x9F\x8D\xAB CANDY PROGRESS",
		total      = 14,                                -- island count
		...
		-- WHITE, because the fill carries a candy-cane stripe GRADIENT and UIGradient
		-- multiplies with BackgroundColor3 -- any tint here would muddy every stripe.
		fillColor  = Color3.fromRGB(255,255,255),
		doneColor  = Color3.fromRGB(255,226,140),       -- stripes turn golden at 14/14
	},

	rows = {
		island = "\xF0\x9F\x8D\xAD Island: ",       -- 🍭 Island:
		height = "\xF0\x9F\x9A\x80 Max Height: ",   -- 🚀 Max Height:
	},
	...
	-- click. Empty = no Sound load, no error. Drop in an OWNED id to give the panel a click.
	uiClickSound = "",
}
...
-- CANDY THEME
--======================================================================
-- Applied over the kit's verbatim Fart to Float look. Only COLOUR changes -- every size and
-- position below is the kit's original number, so the layout, the mobile repositioning and
-- FlightFeedback's docking (it reads RightPanel's live AbsolutePosition/AbsoluteSize) all
-- behave exactly as documented in the README.
--
-- The two body stops are CHOSEN BY CONTRAST, not by eye. White measures 4.6:1 on the top stop
-- and 6.5:1 on the bottom, both clearing WCAG AA (4.5:1); the blue this replaces was 6.3:1.
-- The top stop is the one that matters -- the stats section is the top 175px of the panel, so
-- every number is read against the LIGHTEST end of the gradient. An obvious first draft
-- (238,116,186) measured 2.7:1 and would have been unreadable exactly where it counts.
-- Deepen the bottom stop freely; raise the top one and you break the numbers.
local CANDY = {
	bodyTop    = Color3.fromRGB(196,  66, 148),
	bodyBot    = Color3.fromRGB(158,  48, 140),
	wrapper    = Color3.fromRGB(255, 255, 255), -- the white stroke around the whole sweet
	titleGold  = Color3.fromRGB(255, 214,  92),
	ink        = Color3.fromRGB( 92,  18,  74), -- plum, for text outlines
	subText    = Color3.fromRGB(255, 226, 245),
	trackCream = Color3.fromRGB(255, 246, 236),
	trackEdge  = Color3.fromRGB(214, 176, 196),
	stripeA    = Color3.fromRGB(255, 255, 255),
	stripeB    = Color3.fromRGB(255,  92, 108),
	pctInk     = Color3.fromRGB( 96,  22,  60),
}

-- Hard stops make a BAND; two keypoints far apart make a fade. ColorSequence requires
-- strictly-increasing offsets starting at exactly 0 and ending at exactly 1.
local function candyStripes(a, b, bands)
	local keys, n = {}, bands or 8
	for i = 0, n - 1 do
...
do
	...
	-- OPT OUT OF THE GLOBAL HUD SCALER. ResponsiveUI sweeps every ScreenGui in PlayerGui and
	-- attaches its own UIScale; this panel already does a MORE SPECIFIC pass of its own further
	-- down (the extra 0.78, because RightPanelGui is one of the screen-hogging clusters).
	-- Without this flag the two race on load order: ResponsiveUI skips elements that already
	-- have a UIScale, so if the kit wins the race everything is fine -- but if ResponsiveUI
	-- wins, it owns the UIScale and overwrites the 0.78 with the global scale on every single
	-- viewport change. The opt-out makes the outcome deterministic instead of load-order luck.
	rightGui:SetAttribute("NoResponsiveScale", true) -- ResponsiveUI:37 OPT_OUT
	...
	-- CANDY: white, because the colour is carried by the UIGradient below and UIGradient
	-- MULTIPLIES with BackgroundColor3 -- the kit's blue here would mute every stop.
	rightPanel.BackgroundColor3 = Color3.new(1,1,1)
	rightPanel.ZIndex = 3; rightPanel.Parent = rightGui
end
mkCorner(rightPanel,20); mkStroke(rightPanel,CANDY.wrapper,3) -- corner 20 kept; white wrapper stroke
candyGradient(rightPanel, CANDY.bodyTop, CANDY.bodyBot, 90)

-- GLOSS: the white band that turns a coloured box into a WRAPPED sweet, matching the rail
-- buttons so the HUD reads as one set.
--
-- ZINDEX 0 IS LOAD-BEARING. RightPanelGui runs ZIndexBehavior.Sibling, where a child's ZIndex
-- only orders it against its OWN siblings -- the stats section draws as one unit at the
-- section frame's level. A gloss at ZIndex 4 would paint over the STATS title no matter how
-- high the title's own ZIndex went, because that comparison never happens. Below the section
-- (0 < 1) is the only place it can sit and still read as a shine. A Frame never consumes
-- input, so nothing above it is blocked either way.
do
	local gloss = Instance.new("Frame")
	gloss.Name = "CandyGloss"
	gloss.Size = UDim2.new(1,-16,0,26); gloss.Position = UDim2.new(0,8,0,5)
	gloss.BackgroundColor3 = Color3.new(1,1,1); gloss.BackgroundTransparency = 0.78
	gloss.BorderSizePixel = 0; gloss.ZIndex = 0; gloss.Parent = rightPanel
	mkCorner(gloss, 13)
end
...
statsTitle.BackgroundTransparency = 1; statsTitle.Text = "\xF0\x9F\x8D\xAC CANDY STATS" -- 🍬
statsTitle.Font = Enum.Font.FredokaOne; statsTitle.TextSize = 20 -- the realm's title face (IslandCard)
statsTitle.TextColor3 = CANDY.titleGold
mkStroke(statsTitle, CANDY.ink, 1.6) -- gold on raspberry needs an outline to stay crisp
...
mkStroke(islandLabel, CANDY.ink, 1.6)
...
mkStroke(heightLabel, CANDY.ink, 1.6)
...
spaceRealmTitle.Font = Enum.Font.GothamBold; spaceRealmTitle.TextSize = 16; spaceRealmTitle.TextColor3 = CANDY.subText
mkStroke(spaceRealmTitle, CANDY.ink, 1.4)
...
-- CANDY TRACK: the kit's 10,14,36 is near-black, and on an otherwise bright sweet-shop HUD it
-- reads as a hole punched in the panel. Cream instead, so the bar looks like a sweet in a wrapper.
spaceBarBG.BackgroundColor3 = CANDY.trackCream; spaceBarBG.BorderSizePixel = 0; spaceBarBG.ZIndex = 4; spaceBarBG.Parent = statsSection
mkCorner(spaceBarBG, 9); mkStroke(spaceBarBG, CANDY.trackEdge, 1)
...
-- The stripes live in a GRADIENT, not in BackgroundColor3, because updateSpaceRealmProgress()
-- rewrites that colour on every island unlock. The gradient survives its writes, and because
-- fillColor is white the stripes render true -- then doneColor tints them gold at 14/14.
do
	local fg = Instance.new("UIGradient")
	fg.Name = "CandyPaint"
	fg.Color = candyStripes(CANDY.stripeA, CANDY.stripeB, 8)
	fg.Rotation = 45
	fg.Parent = spaceFill
end
...
-- plum text on a WHITE outline: this label sits on the cream track AND over the stripes, so
-- white-on-white would vanish the moment the bar filled underneath it
spacePctLabel.Font = Enum.Font.GothamBold; spacePctLabel.TextSize = 13; spacePctLabel.TextColor3 = CANDY.pctInk
spacePctLabel.TextScaled = true; spacePctLabel.RichText = false; spacePctLabel.ZIndex = 5; spacePctLabel.Parent = spaceBarBG
mkStroke(spacePctLabel, Color3.new(1,1,1), 2)
