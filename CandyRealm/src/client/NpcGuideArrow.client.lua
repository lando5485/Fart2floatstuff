--======================================================================
-- NpcGuideArrow.client.lua   (LocalScript, per-player)
--======================================================================
...
-- stood on a large island with a person somewhere on it. This lays the green chevron trail
-- from wherever you touch down to that island's NPC, then puts itself away.
--
-- IT DOES NOT DRAW ANYTHING. GardenGuideTrail already owns a chevron trail that works on every
-- island, hides itself mid-flight and comes back when you land -- all the awkward parts are
-- solved in there. This script only decides WHERE it should point, through the low-priority
-- slot _G.guideTrailNpc, so a hatchable egg still beats an NPC every time.
--
...
-- guidance -- the same reason the Bean Farm tutorial arrows end for good on your first flight.
--======================================================================
...
local ISLAND_PREFIX = "island"   -- the models are island1..island14, not Island_1_
...
-- names in this place carry stray spaces, underscores and casing ('gas vents ', 'Candy Npc')
...
--
-- Bounding boxes instead: you are on the island whose footprint you are inside. Boxes are
-- cached because GetBoundingBox walks every part, but a box under ~50 studs across means the
-- island had not streamed in when it was measured, so that one is measured again rather than
-- trusted.
...
-- The NPC INSIDE that island, never a Workspace-wide search. Island 13's fountain and island
-- 14's campfire are only about 550 studs apart, and a nearest-in-range sweep happily wires one
-- island's quest giver to the other -- which is exactly the bug those two quests already hit.
...
-- The trail lives in another script. If that one is missing or is an older copy without the
-- NPC slot, this script does everything right and nothing appears -- so say so, once, loudly,
-- rather than sitting there silently pointing at nobody.
task.delay(5, function()
    if not _G.guideTrailNpc then
        warn("[NpcGuide] GardenGuideTrail has no _G.guideTrailNpc -- arrows CANNOT draw. "
            .. "Make sure GardenGuideTrail.client.luau is synced.")
    end
end)

-- /arrows -- print exactly what the guide can see, so a silent trail can be diagnosed in one
...
RunService.Heartbeat:Connect(function()
    ...
    -- Mid-flight the trail hides itself anyway, so skip the searching too -- but do NOT clear
    -- the target on the flag alone. If _G.isFlying lingers true after landing, clearing here
    -- means the trail never comes back, which is exactly the failure that is impossible to see
    -- from the outside. Keep the target; the trail decides whether to draw it.
    ...
    if now >= nextScan then
        ...
        -- re-find if it streamed out, or if we never found one -- island models arrive before
        -- their contents do, so a single look at landing time finds nothing on most islands
