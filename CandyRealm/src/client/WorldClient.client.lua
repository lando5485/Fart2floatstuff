...
getStandPosition = function(islandNum) -- (forward-declared above for startBubbleDrift)
	...
	-- CandyRealm never sets _G.ISLAND_POS (it's a leftover from the original place), so
	-- indexing it threw "attempt to index nil with number" every session. Fall back to the
	-- island model itself, and finally to nil so callers can skip rather than crash.
	local p = _G.ISLAND_POS and _G.ISLAND_POS[islandNum]
	if p then return Vector3.new(p.x, p.y + 20, p.z) end
	local island = findIsland(islandNum)
	if island then
		local ok, cf = pcall(function() return (select(1, island:GetBoundingBox())) end)
		if ok and cf then return cf.Position + Vector3.new(0, 25, 0) end
	end
	return nil
end
...
task.spawn(function()
	...
	while true do
		...
		pcall(function()
			...
			-- same story: _G.ISLAND_POS doesn't exist here, so read the island's own position
			local tp=_G.ISLAND_POS and _G.ISLAND_POS[nextIsland]
			local target3D = tp and Vector3.new(tp.x,tp.y,tp.z) or getStandPosition(nextIsland)
			if not target3D then navFrame.Visible=false; navName.Visible=false; return end
