#!/usr/bin/env python3
"""
ladder.py -- replay the whole Candy Realm climb from the SHIPPED files and assert the design rules.

The Candy Realm runs the FOOD REALM's progression model (the F2F UNIVERSAL PROGRESSION GUIDE / Dino
tower), ported verbatim: 13 islands on the guide's exact positions, the guide's seven guts, the food
realm's 13 food rows. This is the food realm's tools/ladder.py pointed at Candy's files:

    src/shared/FlightTuning.luau   -- the flight math and coin constants
    src/shared/IslandOrder.luau    -- SLOT_POS (Vector3.new(x, y, z) rows)
    src/shared/StomachTiers.luau   -- CANDY_GUTS (name / maxPower / cost / unlockSlot)
    src/shared/CandyData.luau      -- FOODS (name / price / power / island)
    src/shared/Constants.luau      -- STARTING_COINS

It flies a reference player up all 12 crossings: no events, no bubbles, always buying the best-value
food they have unlocked, and always launching on the fullest tank whole servings can make.

Exit code 0 = every invariant holds. Run it after ANY tuning change:

    python tools/ladder.py            (from CandyRealm/)

If this file and any doc disagree, this file is right -- it reads the code that actually runs.
"""
...
# ---------------------------------------------------------------- parse IslandOrder (Vector3 rows)
io_src = read("src/shared/IslandOrder.luau")
m = re.search(r"IslandOrder\.SLOT_POS\s*=\s*\{(.*?)\n\}", io_src, re.S)
SLOT_Y = [float(y) for _, y, _ in
          re.findall(r"Vector3\.new\(\s*(-?[\d.]+)\s*,\s*(-?[\d.]+)\s*,\s*(-?[\d.]+)\s*\)", m.group(1))]
...
NC = len(GAPS)
m = re.search(r"IslandOrder\.GAPS\s*=\s*\{([^}]*)\}", io_src)
DECLARED_GAPS = nums(m.group(1)) if m else GAPS

# ---------------------------------------------------------------- parse StomachTiers / CandyData / Constants
st = read("src/shared/StomachTiers.luau")
m = re.search(r"StomachTiers\.CANDY_GUTS\s*=\s*\{(.*?)\n\}", st, re.S)
GUTS = [(n, int(mp), int(c), int(u), r == "true") for n, mp, c, u, r in
        re.findall(r'name\s*=\s*"([^"]+)",\s*maxPower\s*=\s*(\d+),\s*cost\s*=\s*(\d+),\s*unlockSlot\s*=\s*(\d+),'
                   r'.*?robux\s*=\s*(true|false)', m.group(1))]
COIN_GUTS = [g for g in GUTS if not g[4]]

cd = read("src/shared/CandyData.luau")
m = re.search(r"CandyData\.FOODS\s*=\s*\{(.*?)\n\}", cd, re.S)
FOOD_ROWS = [(n, int(p), int(w), int(i)) for n, p, w, i in
             re.findall(r'name\s*=\s*"([^"]+)",\s*price\s*=\s*(\d+),\s*power\s*=\s*(\d+),\s*island\s*=\s*(\d+)',
                        m.group(1))]
# the food realm keys foods by island == slot; Candy keys by ROW (slot) and `island` names the host model,
# so rebuild the (name, price, power, SLOT) shape the replay below expects.
FOOD = [(n, p, w, slot + 1) for slot, (n, p, w, _) in enumerate(FOOD_ROWS)]

cst = read("src/shared/Constants.luau")
START_COINS = int(re.search(r"Constants\.STARTING_COINS\s*=\s*(\d+)", cst).group(1))
...
print("TOWER")
chk(len(SLOT_Y) == len(FOOD), "%d islands and %d foods -- one food per slot" % (len(SLOT_Y), len(FOOD)))
chk(len(DECLARED_GAPS) == NC and all(abs(a - b) < 0.01 for a, b in zip(DECLARED_GAPS, GAPS)),
    "IslandOrder.GAPS matches what SLOT_POS actually spaces")
...
# the island gate sits on the island BELOW the gut's wall: slot 2k-2 for tier k (1-based), slot 1 for tier 1
chk(all(COIN_GUTS[k][3] == max(1, 2 * (k + 1) - 2) for k in range(NT)),
    "every gut unlocks on the island below its wall (%s)" % ", ".join(str(g[3]) for g in COIN_GUTS))
...
for k in range(NT):
    ...
    if k < NT - 1:
        nxt_gaps = [GAPS[c] for c in range(NC) if TIER_OF[c] == k + 1]
        if nxt_gaps:
            nxt = min(nxt_gaps)
...
def best_fill(tank, fuel, coins, unlocked, gap):
    """How a sensible player shops, in whole servings:
       ...
       - otherwise BUY MAX the way the shop does: best fuel-per-coin first, as many whole servings as the
         coins and the tank allow, then top up leftover room with the next-best food.
...
for c, gap in enumerate(GAPS):
    ...
    while True:
        if tier < need and coins >= COIN_GUTS[need][2] and unlocked >= COIN_GUTS[need][3]:
