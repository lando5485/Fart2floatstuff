...
local function reqTable(player)
	return { islands = islandsDone(player), space = spaceDone(player.UserId), dino = dinoDone(player.UserId) }
end
...
local function pushState(player)
	...
	pcall(function()
		remote:FireClient(player, "state", {
			...
			canRebirth = reqs.islands and reqs.space and reqs.dino,
		})
	end)
end
...
remote.OnServerEvent:Connect(function(player, action)
	...
	local reqs = reqTable(player)
	if not (reqs.islands and reqs.space and reqs.dino) then
